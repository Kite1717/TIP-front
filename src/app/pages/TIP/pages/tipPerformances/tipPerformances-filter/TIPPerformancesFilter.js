...
const prepareFilter = (queryParams, values) => {
  const { status, type, Kategori, TipAdi } = values;
...
export function TIPPerformancesFilter({ listLoading }) {
  ...
  const applyFilter = (values) => {
    ...
    const newQueryParams = prepareFilter(tipPerformancesUIProps.queryParams, values);
    if (!isEqual(newQueryParams, tipPerformancesUIProps.queryParams)) {
      newQueryParams.pageNumber = 1;
      // update list by queryParams
      tipPerformancesUIProps.setQueryParams(newQueryParams);
    }
  };
  const intl = useIntl()
