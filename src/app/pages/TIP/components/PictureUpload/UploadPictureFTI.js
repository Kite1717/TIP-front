...
const updateFiles = async(result,files) =>{
  ...
  // {
  //   let flag = false
  //   for(let j = 0 ; result.successful.length ;j++)
  //   {
  //     if(   result.successful[j].name === files[i].name)
  //     {
  //       flag = true
  //       break;
  //     }
...
const checkCode = async (result,files) => {
        ...
        for await (let item of files){
          await axios.get(`/fti-library/find/${item.name.split(".")[0]}`).then((res) => {
...
const uploadData = async(result,files) =>{
         ...
         formData.append("catId",   cat)
          formData.append("subCatId",   sub)
          formData.append("IsNonTIP", IsNonTIP)
           axios.post("/fti-library/upload-image", formData)
           j++
          }
}


export function UploadPictureFTI({catId,subCatId,isNonTip}) {
 
 useEffect(() => {
  IsNonTIP = isNonTip
  cat = catId
  sub = subCatId
 }, [catId,subCatId,isNonTip])
  return (
    <>
      <Dashboard
        id="drag-drop"
