...
//fetch user IO Recods

export const fetchUserIORecords = deviceIdNo => dispatch => {

  
  return requestFromServer
    .getUserIORecords(deviceIdNo)
    .then(response => {
      const {totalCount,entities} = response.data;
      dispatch(actions.userIORecordsFetched({ entities,totalCount }));
    })
    .catch(error => {
      error.clientMessage = "Can't find user";
      dispatch(actions.catchError({ error, callType: callTypes.action }));
...
//fetch user monthly infos



export const fetchUserMonthlyReport = deviceIdNo => dispatch => {

  
  return requestFromServer
    .getUserInfosMonthly(deviceIdNo)
    .then(response => {
      const {totalCount,entities} = response.data;
      dispatch(actions.userMonthlyInfoFetched({ entities,totalCount }));
    })
    .catch(error => {
      error.clientMessage = "Can't find user";
      dispatch(actions.catchError({ error, callType: callTypes.action }));
...
//fetch user invidual infos


export const fetchUserIndividualReport = deviceIdNo => dispatch => {

  
  return requestFromServer
    .getUserInfosIndividual(deviceIdNo)
    .then(response => {
      const {totalCount,entities} = response.data;
      
      dispatch(actions.userInvidualInfoFetched({ entities,totalCount }));
    })
    .catch(error => {
      error.clientMessage = "Can't find user";
      dispatch(actions.catchError({ error, callType: callTypes.action }));
